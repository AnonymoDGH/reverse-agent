 ...
 * These are dynamically imported only when `reverse-agent plugin *` or `reverse-agent plugin marketplace *` runs.
 */
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `reverse-agent plugin install` to install a plugin.',
      )
    }
  }
