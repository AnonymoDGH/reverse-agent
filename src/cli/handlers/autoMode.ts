 ...
 * critique user-written rules. Dynamically imported when `reverse-agent auto-mode ...` runs.
 */
...
const CRITIQUE_SYSTEM_PROMPT =
  'You are an expert reviewer of auto mode classifier rules for Reverse Agent.\n' +
  '\n' +
  'Reverse Agent has an "auto mode" that uses an AI classifier to decide whether ' +
  ...
  if (!hasCustomRules) {
    process.stdout.write(
      'No custom auto mode rules found.\n\n' +
        ...
        'Run `reverse-agent auto-mode defaults` to see the default rules for reference.\n',
    )
    return
  }
