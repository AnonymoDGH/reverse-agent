...
export const QWEN_FREE_COSTS: ModelCosts = {
  inputTokens: 0,
  outputTokens: 0,
...
function trackUnknownModelCost(model: string, shortName: ModelShortName): void {
  if (isQwenReverseEnabled()) return
...
export function calculateUSDCost(resolvedModel: string, usage: Usage): number {
  if (isQwenReverseEnabled() || resolvedModel.toLowerCase().startsWith('qwen')) {
    return 0
  }
