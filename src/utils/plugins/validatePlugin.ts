 ...
 * one into the other. Surfaced as warnings by `reverse-agent plugin validate` since
...
export async function validatePluginManifest(
  ...
  // them. `reverse-agent plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Reverse Agent ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Reverse Agent accepts ` +
 ...
 * for the load path, but authors running `reverse-agent plugin validate` want a hard
