 ...
 *   env var is set by `reverse-agent ssh` on the remote CLI to route API calls through
...
export function getProxyFetchOptions(opts?: { forAnthropicAPI?: boolean }): {
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `reverse-agent ssh` auth proxy, which
